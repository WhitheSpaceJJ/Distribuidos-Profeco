...
import entidades.oficial.Multa;
...
public class MultaCola implements AutoCloseable {

    private final com.rabbitmq.client.Connection CONNECTION;
    private final com.rabbitmq.client.Channel CHANNEL;
    private final String REQUEST_QUEUE_NAME = "rpc_queue_multas";

    public MultaCola() throws IOException, TimeoutException {
    ...
    public boolean guardar(Multa message) throws IOException, InterruptedException, ExecutionException {
    ...
    public boolean actualizar(Multa message) throws IOException, InterruptedException, ExecutionException {
        final String corrId = UUID.randomUUID().toString();
    ...
    public Multa obtenerID(int id) throws IOException, InterruptedException, ExecutionException {
        final String corrId = UUID.randomUUID().toString();
        ...
        final CompletableFuture<Multa> response = new CompletableFuture<>();
        ...
        String ctag = CHANNEL.basicConsume(replyQueueName, true, (consumerTag, delivery) -> {
            if (delivery.getProperties().getCorrelationId().equals(corrId)) {
                ...
                if (response2 != null) {
                    try {
                        ObjectMapper mapper = new ObjectMapper();
                        objeto = mapper.readValue(response2, Multa.class);
                    } catch (Exception e) {
        ...
        Multa peticion = response.get();
        CHANNEL.basicCancel(ctag);
        return peticion;
    }

    public Multa[] listar() throws IOException, InterruptedException, ExecutionException {
        final String corrId = UUID.randomUUID().toString();
        ...
        Multa[] peticion = (Multa[]) response.get();
        CHANNEL.basicCancel(ctag);
        return peticion;
    }
